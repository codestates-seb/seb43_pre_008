...
const Ask2 = () => {
  ...
  const getDetail = async (parameter) => {
    //질문 답변 통용
  ...
  useEffect(() => {
    getDetail(`http://localhost:4000/discussions/8`).then((res) => {
  ...
  const onChangeSummary = (e) => {
    let isFit = e.target.value.length > 10;
    setBody({ ...body, summary: e.target.value });
    if (!isFit) {
      setTitleError(true);
    } else {
      setTitleError(false);
